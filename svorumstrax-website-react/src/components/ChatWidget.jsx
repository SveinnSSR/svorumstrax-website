...
const MessageFormatter = ({ message }) => {
  ...
  if (
    paragraphs.length <= 1 &&
    !message.includes("https://www.google.com/maps/")
  ) {
    return <>{message}</>;
  }
...
const ChatWidget = () => {
  ...
  const translations = {
    is: {
      ...
      send: "Senda",
      welcome:
        "Hæ! Ég er AI spjallmenni hjá Svörum strax. Ertu með fyrirtæki og hefur áhuga á þjónustu okkar? Eða hefur þú áhuga á að ganga til liðs við okkur í Barcelona?",
    ...
    en: {
      ...
      send: "Send",
      welcome:
        "Hello! I'm your AI assistant at Svörum strax. Are you a business interested in our services? Or are you looking to join our team in Barcelona?",
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch(
        "https://svorumstrax-chatbot-api.vercel.app/api/",
        {
          method: "POST",
          headers: {
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <path
                d="M28 20a2.67 2.67 0 0 1-2.67 2.67H9.33L4 28V6.67A2.67 2.67 0 0 1 6.67 4h18.66A2.67 2.67 0 0 1 28 6.67V20z"
                fill={WIDGET_THEME.color}
              />
                    ...
                    {msg.type === "bot" ? (
                      typingMessages[msg.id || ""] ? (
                        <div
                          style={{
                            position: "relative",
                            opacity: typingMessages[msg.id || ""].fadeIn
                              ? "0.99"
                              : "1",
                        ...
                        >
                          ...
                          >
                            <MessageFormatter
                              message={typingMessages[msg.id || ""].text}
                            />
                          </div>

                          <div
                            style={{
                              animation: typingMessages[msg.id || ""].fadeIn
                                ? `fadeIn ${FADE_IN_DURATION}ms ease-in-out`
                                : "none",
                            }}
                          >
                            <MessageFormatter
                              message={typingMessages[
                                msg.id || ""
                              ].text.substring(
                                0,
                                typingMessages[msg.id || ""].visibleChars,
                              )}
                            />
                          </div>
                        </div>
